...
module.exports = function (WhatsAppWeb) {
	...
	WhatsAppWeb.prototype.sendMediaMessage = function (
		...
		const fileEncSha256B64 = Utils.sha256(body).toString('base64').replace(/\+/g, '-').replace(/\//g, '_')
			.replace(/\=+$/, '');
		...
		const promise = Utils.generateThumbnail(buffer, mediaType, info)
			...
			.then((json) => {
				...
				return fetch(hostname, { method: 'POST', body, headers: { Origin: 'https://web.whatsapp.com' } });
			})
			.then((res) => res.json())
			.then((json) => {
				if (json.url) {
