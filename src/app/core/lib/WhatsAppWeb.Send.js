...
module.exports = function (WhatsAppWeb) {
  ...
  WhatsAppWeb.prototype.sendReadReceipt = function (jid, messageID) {
    const json = [
      ...
      [['read', {
        count: '1', index: messageID, jid, owner: 'false',
      }, null]],
    ];
  ...
  // optionally you can provide a quoted message & the timestamp for the message
  ...
  // send a media message to someone, optionally you can provide a caption,
  // thumbnail, mimetype & the timestamp for the message
  WhatsAppWeb.prototype.sendMediaMessage = function (
    id,
    buffer,
    mediaType,
    info = null,
    ...
    if (mediaType === WhatsAppWeb.MessageType.text
			|| mediaType === WhatsAppWeb.MessageType.extendedText) {
      return Promise.reject(new Error('use sendTextMessage() to send text messages'));
    }
    if (mediaType === WhatsAppWeb.MessageType.document && !info.mimetype) {
      return Promise.reject(new Error('mimetype required to send a document'));
    }
    if (mediaType === WhatsAppWeb.MessageType.sticker && info.caption) {
      return Promise.reject(new Error('cannot send a caption with a sticker'));
    }
    if (!info.mimetype) {
    ...
    const promise = Utils.generateThumbnail(buffer, mediaType, info)
    // send a query JSON to obtain the url & auth token to upload our media
      .then(() => this.query(['query', 'mediaConn']))
      .then((json) => {
        const { auth } = json; // the auth token
        ...
        return fetch(
          hostname,
          { method: 'POST', body, headers: { Origin: 'https://web.whatsapp.com' } },
        );
      })
      .then((res) => res.json())
      .then((json) => {
  ...
  WhatsAppWeb.prototype.sendBinary = function (json, tags) {
    ...
    // encrypt it using AES and our encKey
    let buff = Utils.aesEncrypt(binary, this.authInfo.encKey);
    // sign the message using HMAC and our macKey
    const sign = Utils.hmacSign(buff, this.authInfo.macKey);
